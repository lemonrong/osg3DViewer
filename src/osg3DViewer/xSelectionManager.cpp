...
xSelectionManager::xSelectionManager()
{
    ...
    m_selectionDecorator = new xDefaultSelectionDecorator;
}
...
void xSelectionManager::clearSelection()
{
    ...
    while (m_selectionDecorator->getNumChildren() > 0)
        m_selectionDecorator->removeChild(0,1);

    if (m_selectionDecorator->getNumParents() > 0)
        m_selectionDecorator->getParent(0)->removeChild(m_selectionDecorator.get());
}
...
bool xSelectionManager::select(osg::Node* pNode)
{
    ...
    if (pNode)
    {
        m_selectionDecorator->addChild(pNode);

        osg::Group* pParent = pNode->getParent(0);

        if (pParent)
        {
            pParent->addChild(m_selectionDecorator.get());
        }
    }
...
bool xSelectionManager::select(osg::Geometry* pGeom)
{
    ...
    if (pGeom)
    {
        ...
        m_selectionDecorator->addChild(newparent);

        osg::Node* pParent = pGeom->getParent(0);

        if (pParent)
            pParent->getParent(0)->addChild(m_selectionDecorator.get());
    }

    // keep the pointer of the selected node
    m_selectedGeometry = pGeom;
...
void xSelectionManager::setSelectionDecorator(xSelectionDecorator* pDecorator)
{
    ...
    m_selectionDecorator = pDecorator;
    select(m_selectedNode.get());
}
...
{
    return m_selectionDecorator.get();
}
